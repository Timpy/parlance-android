...
//	private boolean mTestDelayElapsed; // add a timer for testing
	...
	public void displayMessageNotification(String fromSipUri, String fromName, String message) {
		/*Intent notifIntent = new Intent(this, ParlanceActivity.class);
	...
	private Class<? extends Activity> incomingReceivedActivity = ParlanceActivity.class;
		...
		mHandler.post(new Runnable() {
			public void run() {
				if (ParlanceActivity.isInstanciated()) {
					ParlanceActivity.instance().onRegistrationStateChanged(state);
				}
			}
		});
	}
