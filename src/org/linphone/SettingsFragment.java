...
import org.hw.parlance.SetupActivity;
...
public class SettingsFragment extends PreferencesListFragment implements EcCalibrationListener {
		...
		findPreference(getString(R.string.menu_about_key)).setOnPreferenceClickListener(new OnPreferenceClickListener() {
			...
			public boolean onPreferenceClick(Preference preference) {
				if (ParlanceActivity.isInstanciated()) {
					ParlanceActivity.instance().displayAbout();
					return true;
				}
				return false;
			}
	...
	private void initAccounts() {
		...
		for (int i = 0; i < nbAccounts; i++) {
			...
			{
				public boolean onPreferenceClick(Preference preference) {
					ParlanceActivity.instance().displayAccountSettings(accountId);
					return false;
				}
			});
	...
	public void onResume() {
		...
		if (ParlanceActivity.isInstanciated()) {
			ParlanceActivity.instance().selectMenu(FragmentsAvailable.SETTINGS);
		}
	}
}
