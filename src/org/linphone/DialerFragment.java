...
public class DialerFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, 
		...
		if (ParlanceActivity.isInstanciated() && LinphoneManager.getLc().getCallsNb() > 0) {
		...
		addContactListener = new OnClickListener() {
			...
			public void onClick(View v) {
				//ParlanceActivity.instance().displayContactsForEdition(mAddress.getText().toString());
			}
		};
		cancelListener = new OnClickListener() {
			@Override
			public void onClick(View v) {
				ParlanceActivity.instance().resetClassicMenuLayoutAndGoBackToCallIfStillRunning();
			}
		};
		...
		mAddContact.setEnabled(!(ParlanceActivity.isInstanciated() && LinphoneManager.getLc().getCallsNb() > 0));
	...
	public void onAttach(Activity activity) {
		...
		if (ParlanceActivity.isInstanciated()) {
			ParlanceActivity.instance().updateDialerFragment(this);
		}
	}
	...
	public void onResume() {
		super.onResume();
		if (ParlanceActivity.isInstanciated()) {
			ParlanceActivity.instance().selectMenu(FragmentsAvailable.DIALER);
			ParlanceActivity.instance().updateDialerFragment(this);
		}
